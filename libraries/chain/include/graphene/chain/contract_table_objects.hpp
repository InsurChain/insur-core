...
namespace config {
   ...
   struct billable_size<table_id_object> {
      static const uint64_t overhead = overhead_per_row_per_index_ram_bytes * 2;  
      static const uint64_t value = 44 + overhead; 
   };
   
   template<>
   struct billable_size<index_double_object> {
      static const uint64_t overhead = overhead_per_row_per_index_ram_bytes * 3;  
      static const uint64_t value = 24 + 8 + overhead; 
   };

   template<>
   struct billable_size<key_value_object> {
      static const uint64_t overhead = overhead_per_row_per_index_ram_bytes * 2;  
      static const uint64_t value = 32 + 8 + 4 + overhead; 
   };
   ...
   struct billable_size<index128_object> {
      static const uint64_t overhead = overhead_per_row_per_index_ram_bytes * 3; 
      static const uint64_t value = 24 + 16 + overhead; 
   };

   template<>
   struct billable_size<index256_object> {
      static const uint64_t overhead = overhead_per_row_per_index_ram_bytes * 3;  
      static const uint64_t value = 24 + 32 + overhead; 
   };
...
FC_REFLECT(graphene::chain::table_id_object, (code)(scope)(table)(payer)(count) )
FC_REFLECT(graphene::chain::key_value_object, (primary_key)(payer)(value) )

#define REFLECT_SECONDARY(type)\
  FC_REFLECT(type, (primary_key)(payer)(secondary_key) )

REFLECT_SECONDARY(graphene::chain::index64_object)
REFLECT_SECONDARY(graphene::chain::index128_object)
REFLECT_SECONDARY(graphene::chain::index256_object)
REFLECT_SECONDARY(graphene::chain::index_double_object)
REFLECT_SECONDARY(graphene::chain::index_long_double_object)
