...
{ try {
        alliance_data_product_object lpo;
        v.as<alliance_data_product_object>(lpo, 20);
...
{ try {
    ...
    if (op.alliance_id.valid()) {
        if (auto obj = db().find(*op.alliance_id)) {
            auto iter = find_if(obj->members.begin(), obj->members.end(),
                    [&](const account_id_type& alliance_member) {
                        return alliance_member == op.requester;
                    });
            FC_ASSERT(iter != obj->members.end(), "requester ${r} is not alliance member!", ("r", op.requester));
        } else {
            elog("alliance_id not found");
            FC_THROW("alliance_id not found");
        }
    }
    return void_result();
...
{ try {
   ...
   if (op.alliance_id.valid()) {
       auto alliance = db().get(*op.alliance_id);
       for (auto& member : alliance.members) {
   ...
   const auto& new_object = db().create<data_transaction_object>([&](data_transaction_object& obj) {
       ...
       if (op.alliance_id.valid()) {
           obj.alliance_id = *op.alliance_id;
       } else {
           obj.alliance_id = fc::optional<alliance_id_type>();
       }
